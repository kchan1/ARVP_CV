...
int main(int argc,char*argv[])
{
  ...
  gsl_matrix*img = gsl_matrix_calloc(100,100);
  ...
  convolution(img,gauss,gauss->size1/2,gauss->size2/2,COLOR_WHITE);
  std::cout<<"Image Diagonal:\n";
  for(unsigned int i=0;i<img->size1&&i<img->size2;i++)
    printf("\timg[%i,%i]=%i\n",i,i,int(gsl_matrix_get(img,i,i)));

  //img buffer for later use
  unsigned char img_int[100*100];
  for(int i=0;i<100;i++)
    for(int j=0;j<100;j++)
      img_int[i+j*100] = gsl_matrix_get(img,j,i);
  ...
  printf("Error Set\n");
  cinfo.image_width = 100;      /* image width and height, in pixels */
  ...
  row_stride = 100;	/* JSAMPLEs per row in image_buffer */
  printf("Scanning... ");
